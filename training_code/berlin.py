...
def downscale_SN3(ipath, size:int):
...
def define_model_SN3():
   #
   # Creating Dataloaders
   #
   tiles = DataBlock(
      blocks = (ImageBlock(),MaskBlock(codes=CODES)), 
      get_items = get_image_tiles_SN3,
      get_y = get_y_SN3,                                                                      
    )   
   dls = tiles.dataloaders(pathSN3, bs=BATCH_SIZE_SN3, num_workers=0)
   dls.vocab = CODES
   print(len(dls.train_ds), len(dls.valid_ds))
   inputs, targets = dls.one_batch()
   print(inputs.shape, targets.shape)
   ...
   #print(learn.summary())
   #print(learn.model)
   learn.export(fname='berlin4.pkl')
   return learn, dls
...
if __name__ == "__main__":
    ...
    #downscale_SN3(r'C:\Users\Timbo\Documents\Projet\multi\data\spacenet3\8bit', 1000)
    #model, loader = define_model_SN3()
    #train_rocky(model, loader)
